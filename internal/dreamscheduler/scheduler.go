...
const (
	DefaultDreamType      = "consolidation"
	DefaultMinConclusions = 50
	DefaultCooldown       = 8 * time.Hour
	DefaultIdleTimeout    = time.Hour
)

// ScheduleConfig carries host/service state needed to schedule local dream work
// without coupling this internal module to the root Service type.
...
func Schedule(ctx context.Context, cfg ScheduleConfig, params DreamScheduleParams) (DreamScheduleResult, error) {
	cfg = normalizeScheduleConfig(cfg)
	if err := ctx.Err(); err != nil {
		return DreamScheduleResult{}, err
	}
	if cfg.DB == nil {
		return DreamScheduleResult{}, fmt.Errorf("goncho: nil db")
	}
	peer := strings.TrimSpace(params.Peer)
	if peer == "" {
		return DreamScheduleResult{}, fmt.Errorf("goncho: peer is required")
	}
	now := EffectiveNow(params.Now)
	base := DreamScheduleResult{
		Action:         "rejected",
		WorkspaceID:    cfg.WorkspaceID,
		ObserverPeerID: cfg.ObserverPeerID,
		ObservedPeerID: peer,
		MinConclusions: cfg.MinConclusions,
	}
	if !cfg.DreamEnabled {
		base.Evidence = DisabledEvidence(cfg.WorkspaceID, cfg.ObserverPeerID, peer)
		return base, nil
	}
	present, err := TableExists(ctx, cfg.DB, "goncho_dreams")
	if err != nil {
		return DreamScheduleResult{}, err
	}
	if !present {
		base.Evidence = UnavailableEvidence(cfg.WorkspaceID, cfg.ObserverPeerID, peer)
		return base, nil
	}

	active, err := findActiveIntent(ctx, cfg.DB, cfg.WorkspaceID, cfg.ObserverPeerID, peer)
	if err != nil {
		return DreamScheduleResult{}, err
	}
	if active != nil {
		return ResultFromIntent("reused", *active), nil
	}

	eligibility, err := readEligibility(ctx, cfg.DB, cfg.WorkspaceID, cfg.ObserverPeerID, peer, now, cfg.IdleTimeout, cfg.Cooldown)
	if err != nil {
		return DreamScheduleResult{}, err
	}
	...
	if !params.Manual {
		if eligibility.newConclusions < cfg.MinConclusions {
			base.Evidence = DreamStatusEvidence{
				Code:             "dream_threshold",
				Reason:           fmt.Sprintf("dream requires at least %d new conclusions since the last completed dream", cfg.MinConclusions),
				WorkspaceID:      cfg.WorkspaceID,
				ObserverPeerID:   cfg.ObserverPeerID,
				ObservedPeerID:   peer,
	...
	intent := Intent{
		...
		ObserverPeerID:   cfg.ObserverPeerID,
		ObservedPeerID:   peer,
		WorkUnitKey:      WorkUnitKey(cfg.WorkspaceID, cfg.ObserverPeerID, peer),
		Status:           "pending",
		NewConclusions:   eligibility.newConclusions,
		MinConclusions:   cfg.MinConclusions,
		LastConclusionID: eligibility.maxConclusionID,
	...
	id, err := insertIntent(ctx, cfg.DB, intent, now, params.Manual, reason)
	if err != nil {
		active, findErr := findActiveIntent(ctx, cfg.DB, cfg.WorkspaceID, cfg.ObserverPeerID, peer)
	...
	intent.ID = id
	return ResultFromIntent("created", intent), nil
}

func CancelPendingForObserved(ctx context.Context, db *sql.DB, workspaceID, observed string, now int64, reason string) (int64, error) {
	present, err := TableExists(ctx, db, "goncho_dreams")
	if err != nil {
		return 0, err
	}
	if !present {
	...
	res, err := db.ExecContext(ctx, `
		...
		WHERE workspace_id = ?
		  ...
		  AND status = 'pending'
	`, reason, now, now, workspaceID, observed)
	if err != nil {
...
func ResultFromIntent(action string, intent Intent) DreamScheduleResult {
	return DreamScheduleResult{
		...
		Evidence:         EvidenceFromIntent(intent),
	}
}

func EvidenceFromIntent(intent Intent) DreamStatusEvidence {
...
func readEligibility(ctx context.Context, db *sql.DB, workspaceID, observer, peer string, now int64, idleTimeout, cooldown time.Duration) (dreamEligibility, error) {
	if idleTimeout <= 0 {
	...
	last, err := latestCompleted(ctx, db, workspaceID, observer, peer, cooldown)
	if err != nil {
	...
	if last != nil && last.CooldownUntil <= 0 {
		out.cooldownUntil = last.LastActivityAt + int64(cooldown.Seconds())
	}
	_ = now
	return out, nil
}

func latestCompleted(ctx context.Context, db *sql.DB, workspaceID, observer, peer string, cooldown time.Duration) (*Intent, error) {
	var intent Intent
	...
	if intent.CooldownUntil <= 0 && completedAt.Valid {
		intent.CooldownUntil = completedAt.Int64 + int64(cooldown.Seconds())
	}
	return &intent, nil
}

func findActiveIntent(ctx context.Context, db *sql.DB, workspaceID, observer, peer string) (*Intent, error) {
	var intent Intent
	err := db.QueryRowContext(ctx, `
		SELECT id, workspace_id, observer_peer_id, observed_peer_id, work_unit_key,
		       status, new_conclusions, min_conclusions, last_conclusion_id,
...
func insertIntent(ctx context.Context, db *sql.DB, intent Intent, now int64, manual bool, reason string) (int64, error) {
	manualInt := 0
	if manual {
		manualInt = 1
	}
...
func DisabledEvidence(workspaceID, observer, peer string) DreamStatusEvidence {
	return DreamStatusEvidence{
		Code:           "dream_disabled",
...
func UnavailableEvidence(workspaceID, observer, peer string) DreamStatusEvidence {
	return DreamStatusEvidence{
...
func WorkUnitKey(workspaceID, observer, peer string) string {
	return "dream:" + DefaultDreamType + ":" + workspaceID + ":" + observer + ":" + peer
}

func EffectiveNow(now time.Time) int64 {
	if now.IsZero() {
		now = time.Now()
	}
	return now.UTC().Unix()
...
func TableExists(ctx context.Context, db *sql.DB, name string) (bool, error) {
	if db == nil {
		return false, fmt.Errorf("goncho: nil db")
	}
	var found string
...
func ScanIntent(row IntentScanner) (Intent, error) {
	...
	if err := row.Scan(
		...
		return Intent{}, fmt.Errorf("goncho: scan dream intent: %w", err)
	}
	return intent, nil
}

func normalizeScheduleConfig(cfg ScheduleConfig) ScheduleConfig {
	cfg.WorkspaceID = strings.TrimSpace(cfg.WorkspaceID)
	cfg.ObserverPeerID = strings.TrimSpace(cfg.ObserverPeerID)
	if cfg.IdleTimeout <= 0 {
		cfg.IdleTimeout = DefaultIdleTimeout
	}
	if cfg.MinConclusions <= 0 {
		cfg.MinConclusions = DefaultMinConclusions
	}
	if cfg.Cooldown <= 0 {
