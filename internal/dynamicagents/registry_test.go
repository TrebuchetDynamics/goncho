...
func newTestDynamicAgentRegistry(t *testing.T) (*dynamicagents.DynamicAgentRegistry, *memory.SqliteStore, string) {
	t.Helper()
	dir := t.TempDir()
	...
	reg, err := dynamicagents.NewDynamicAgentRegistry(store.DB())
	if err != nil {
		t.Fatalf("dynamicagents.NewDynamicAgentRegistry: %v", err)
	}
	t.Cleanup(func() {
...
// same dynamicagents.AgentRecord with a stable ID and persona seed across a registry
...
func TestDynamicAgentRegistry_CreateRoundTrips(t *testing.T) {
	...
	created, err := reg.Create(ctx, dynamicagents.CreateAgentOptions{
		Name:    "Research Bot",
	...
	reg2, err := dynamicagents.NewDynamicAgentRegistry(reopened.DB())
	if err != nil {
		t.Fatalf("reopen dynamicagents.NewDynamicAgentRegistry: %v", err)
	}
...
func TestDynamicAgentRegistry_BindResolvesByPeer(t *testing.T) {
	...
	match := dynamicagents.BindingMatch{
		Channel:  "telegram",
		PeerKind: "group",
		PeerID:   "-100123",
		ThreadID: "7",
...
// config (passed via dynamicagents.CreateAgentOptions.ReservedIDs). Operator-defined
...
func TestDynamicAgentRegistry_StaticConfigWinsOnIDConflict(t *testing.T) {
	...
	_, err := reg.Create(ctx, dynamicagents.CreateAgentOptions{
		Name:        "Research",
		Persona:     "literature review",
		ReservedIDs: reserved,
	})
	if !errors.Is(err, dynamicagents.ErrAgentIDReserved) {
		t.Fatalf("Create with reserved id err = %v, want %v", err, dynamicagents.ErrAgentIDReserved)
	}
	...
	other, err := reg.Create(ctx, dynamicagents.CreateAgentOptions{
		Name:        "Triage",
...
func TestDynamicAgentRegistry_UnbindRemovesMatch(t *testing.T) {
	...
	created, err := reg.Create(ctx, dynamicagents.CreateAgentOptions{Name: "Research"})
	if err != nil {
		t.Fatalf("Create: %v", err)
	}
	match := dynamicagents.BindingMatch{
