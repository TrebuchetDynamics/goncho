...
func TestHostIntegrationMappingSupportsDocumentedSessionStrategies(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := hostintegration.Map(tt.input)
...
func TestHostIntegrationRecallModesMapContextToolsAndHybrid(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.mode, func(t *testing.T) {
			got := hostintegration.Map(hostintegration.Input{
				Host:             "opencode",
				PeerName:         "alice",
				SessionStrategy:  "per-directory",
				WorkingDirectory: "/work/acme/frontend",
...
func TestConfigPatchScopesWritesToSelectedHost(t *testing.T) {
	doc := hostintegration.ConfigDocument{
		APIKey:   "hch-shared",
		BaseURL:  "http://127.0.0.1:8000",
		PeerName: "alice",
		Hosts: map[string]hostintegration.RuntimeConfig{
			"opencode": {
				Workspace:       "opencode",
				AIPeer:          "opencode",
				RecallMode:      "hybrid",
	...
	updated, err := hostintegration.ApplyConfigPatch(doc, "opencode", hostintegration.ConfigPatch{
...
func TestConfigDocumentJSONUsesHonchoSharedKeys(t *testing.T) {
	raw, err := json.Marshal(hostintegration.ConfigDocument{
		APIKey:   "hch-shared",
		BaseURL:  "http://127.0.0.1:8000",
		PeerName: "alice",
		Hosts: map[string]hostintegration.RuntimeConfig{
...
func TestHonchoExternalCompatibilityKeepsGonchoInternalName(t *testing.T) {
	got := hostintegration.HonchoExternalCompatibility()
...
func hasUnsupportedField(items []hostintegration.UnsupportedMapping, field string) bool {
