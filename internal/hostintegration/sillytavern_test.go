...
func TestSillyTavernPeerModesMapSharedAndPersonaScopedUserPeers(t *testing.T) {
	shared := hostintegration.MapSillyTavern(hostintegration.SillyTavernInput{
	...
	separate := hostintegration.MapSillyTavern(hostintegration.SillyTavernInput{
...
func TestSillyTavernSessionNamingModesAndResetEvidence(t *testing.T) {
	tests := []struct {
		name            string
		input           hostintegration.SillyTavernInput
		wantSession     string
		wantOrphaned    string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := hostintegration.MapSillyTavern(tt.input)
...
func TestSillyTavernGroupChatMapsOnePeerPerCharacterAndLazyAddsSpeakers(t *testing.T) {
	got := hostintegration.MapSillyTavern(hostintegration.SillyTavernInput{
		PeerName:                 "alice",
		SessionNaming:            "auto",
...
func TestSillyTavernEnrichmentModesAndUnsupportedPanelEvidence(t *testing.T) {
	...
	degraded := hostintegration.MapSillyTavern(hostintegration.SillyTavernInput{
		PeerName:              "alice",
		SessionNaming:         "auto",
		ChatInstanceID:        "chat-1",
