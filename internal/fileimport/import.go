...
// Params is the local Goncho equivalent of Honcho's multipart file
...
// Result describes the ordinary session messages written from an
...
// UnavailableEvidence names import capabilities accepted in the request that
// cannot yet be fulfilled by the current local storage model.
...
// Metadata mirrors Honcho's file-related internal metadata attached
...
// Import converts a text-like file into ordinary ready user turns for the
...
func Import(ctx context.Context, opts Options, params Params) (Result, error) {
	if opts.DB == nil {
		return Result{}, errors.New("goncho: file import requires a non-nil *sql.DB")
	}
	sessionKey := strings.TrimSpace(params.SessionKey)
	if sessionKey == "" {
		return Result{}, fmt.Errorf("goncho: session_key is required")
	}
	peerID := strings.TrimSpace(params.PeerID)
	if peerID == "" {
		return Result{}, fmt.Errorf("goncho: peer_id is required")
	}
	contentType := normalizeContentType(params.ContentType)
	if contentType == "" {
		return Result{}, fmt.Errorf("goncho: content_type is required")
	}
	if opts.MaxFileSize > 0 && len(params.Content) > opts.MaxFileSize {
		return Result{}, fmt.Errorf("goncho: file size %d exceeds maximum %d", len(params.Content), opts.MaxFileSize)
	}
	...
	if err != nil {
		return Result{}, fmt.Errorf("goncho: begin file import: %w", err)
	}
	...
	for i, chunk := range chunks {
		...
		if err != nil {
			return Result{}, fmt.Errorf("goncho: insert imported file message: %w", err)
		}
		id, err := res.LastInsertId()
		if err != nil {
			return Result{}, fmt.Errorf("goncho: imported file message id: %w", err)
		}
	...
	if err := tx.Commit(); err != nil {
		return Result{}, fmt.Errorf("goncho: commit file import: %w", err)
	}

	unavailable := []UnavailableEvidence{queueUnavailableEvidence()}
	if quarantined {
...
func marshalImportMeta(file Metadata, metadata, configuration map[string]any) (string, error) {
	meta := map[string]any{
		"file_id":               file.FileID,
...
func promptInjectionQuarantineEvidence() UnavailableEvidence {
...
func queueUnavailableEvidence() UnavailableEvidence {
	return UnavailableEvidence{
		Field:      "queue",
