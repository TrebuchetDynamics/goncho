...
import (
	...
	"github.com/TrebuchetDynamics/goncho/internal/fileimport"
	memory "github.com/TrebuchetDynamics/goncho/memory"
)

const testDefaultMaxMessageSize = 25_000

func newTestFileImportDB(t *testing.T) *sql.DB {
	t.Helper()
	store, err := memory.OpenSqlite(t.TempDir()+"/file-import.db", 0, nil)
	if err != nil {
...
func TestService_ImportFileCreatesSessionMessagesWithFileMetadata(t *testing.T) {
	...
	got, err := fileimport.Import(context.Background(), fileimport.Options{DB: db, WorkspaceID: "default", DefaultMaxMessageSize: testDefaultMaxMessageSize}, fileimport.Params{
	...
	rows := loadImportedTurns(t, db, "session-import-1")
	if len(rows) != 1 {
...
func TestService_ImportFileRejectsUnsupportedTypesBeforeWrites(t *testing.T) {
	...
	rows := loadImportedTurns(t, db, "session-import-unsupported")
	if len(rows) != 0 {
...
func TestService_ImportFileRequiresPeerIDBeforeWrites(t *testing.T) {
	...
	rows := loadImportedTurns(t, db, "session-import-missing-peer")
	if len(rows) != 0 {
...
func TestService_ImportFileChunksAtHonchoRuntimeLimit(t *testing.T) {
	...
	content := strings.Repeat("a", testDefaultMaxMessageSize) + strings.Repeat("b", 10)
	got, err := fileimport.Import(context.Background(), fileimport.Options{DB: db, WorkspaceID: "default", DefaultMaxMessageSize: testDefaultMaxMessageSize}, fileimport.Params{
	...
	if len(got.Messages[0].Content) != testDefaultMaxMessageSize {
		t.Fatalf("first chunk len = %d, want %d", len(got.Messages[0].Content), testDefaultMaxMessageSize)
	}
	if got.Messages[0].File.ChunkCharacterRange != [2]int{0, testDefaultMaxMessageSize} {
	...
	if got.Messages[1].File.ChunkCharacterRange != [2]int{testDefaultMaxMessageSize, testDefaultMaxMessageSize + 10} {
...
func TestService_ImportFileDoesNotPersistOriginalFileBytes(t *testing.T) {
	...
	dump := dumpSessionRows(t, db, "session-import-json")
