...
func BuildPairedOutcomes(report goncho.RecallBenchmarkReport, configID string, runStartedAt time.Time, score scoring.CaseScoreFunc) []filecontract.PairedOutcome {
	out := make([]filecontract.PairedOutcome, 0, len(report.Cases))
	started := shared.FormatArtifactTimestamp(runStartedAt)
	for _, c := range report.Cases {
		fields := artifactcontract.BuildCaseFields(c)
		caseScore := score(c)
		out = append(out, filecontract.PairedOutcome{
			ConfigID:       configID,
...
func BuildFailureAuditRows(report goncho.RecallBenchmarkReport, configID string, runStartedAt time.Time) []filecontract.FailureAuditRow {
	out := []filecontract.FailureAuditRow{}
	started := shared.FormatArtifactTimestamp(runStartedAt)
	for _, c := range report.Cases {
		...
		fields := artifactcontract.BuildCaseFields(c)
		out = append(out, filecontract.FailureAuditRow{
			ConfigID:              configID,
			RunStartedAt:          started,
			Scale:                 fields.Scale,
			ConversationID:        fields.ConversationID,
