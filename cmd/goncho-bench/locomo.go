...
type locomoLeakageChecks struct {
	AnswerHintTextPresentInMemory int      `json:"answer_hint_text_present_in_memory"`
...
func writeLocomoMarkdown(path string, report locomoReport, jsonPath, failurePath string) error {
	...
	b.WriteString("# LOCOMO Smoke Retrieval Report\n\n")
	...
	fmt.Fprintf(&b, "- Memories fixture: `%s`\n", report.FixturePaths.Memories)
	fmt.Fprintf(&b, "- Questions fixture: `%s`\n", report.FixturePaths.Questions)
...
func checkLocomoLeakage(data locomoDataset) locomoLeakageChecks {
	...
	for _, q := range data.Questions {
		hint := strings.TrimSpace(strings.ToLower(q.AnswerHint))
		if hint == "" {
			continue
		}
		for _, mem := range byConversation[q.ConversationID] {
			if strings.Contains(strings.ToLower(locomoIndexableContent(mem)), hint) {
				checks.AnswerHintTextPresentInMemory++
				if len(checks.Examples) < 5 {
					checks.Examples = append(checks.Examples, q.QuestionID+":"+mem.MemoryID)
				}
			}
		}
	}
