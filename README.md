...
- **Runnable reference:** pkg.go.dev renders the compiled `NewService` example from this module, so the embedded-service shape is checked by `go test` instead of drifting as prose.
