...
func TestServiceSearchFactIntentRanksPastTimelineAnswerOverLexicalEcho(t *testing.T) {
	...
	answer, err := svc.Conclude(ctx, ConcludeParams{
		Peer:       "team",
		Conclusion: "Release Orion was on 2026-06-01.",
		SessionKey: "sess-past-timeline-intent",
	})
	if err != nil {
		t.Fatal(err)
	}
	decoy, err := svc.Conclude(ctx, ConcludeParams{
		Peer:       "team",
		Conclusion: "When was Release Orion? Release Orion when Release Orion when Release Orion when. This checklist repeats the retrieval words but does not state the date.",
		SessionKey: "sess-past-timeline-intent",
	})
	if err != nil {
		t.Fatal(err)
	...
	if got.Results[0].ID != answer.ID || !strings.Contains(got.Results[0].Content, "2026-06-01") {
		t.Fatalf("first result = %+v, want past timeline fact before lexical echo", got.Results[0])
	}
}
