...
func TestRecallPipelineSpeakerNoteKeepsMultiWordIdentity(t *testing.T) {
	...
	engine := newRecallPipelineEngine(staticRecallGenerator{candidates: []RecallCandidate{
		{
			...
			Provenance: []EvidenceItem{
				...
				{Kind: "speaker", Note: "speaker=Juan Perez source=meeting"},
			},
		},
		{
			...
			Provenance: []EvidenceItem{
				...
				{Kind: "speaker", Note: "speaker=Juan Lopez source=meeting"},
			},
		},
	}}, recallPipelineOptions{
		pipelineVersion: "speaker-note-multi-word-test-v1",
		scoringConfig: RecallScoringConfig{
			Version:     "speaker-note-multi-word-test-v1",
			Weights:     map[string]float64{"keyword": 1},
			RRFK:        60,
			MMRLambda:   1,
			TokenBudget: 80,
	...
	if !slices.Equal(selectedRecallIDs(trace), []string{"mem-juan-perez-note"}) {
		t.Fatalf("selected IDs = %v, want multi-word speaker note to disambiguate same first-name speakers", selectedRecallIDs(trace))
	}
	if !strings.Contains(strings.Join(trace.Selected[0].Score.WhySelected, "\n"), "speaker_adjustment=0.120000") {
		t.Fatalf("why selected = %v, want speaker adjustment evidence from multi-word speaker note", trace.Selected[0].Score.WhySelected)
	}
}
