...
func TestServiceSearchUserScopeReturnsLineageEvidenceForWidenedHits(t *testing.T) {
	...
	if got.ScopeEvidence == nil ||
		got.ScopeEvidence.Decision != CrossChatDecisionAllowed ||
...
func TestService_SearchUserScopeUnknownCurrentBindingFallsBackSameSession(t *testing.T) {
	...
	ctx := context.Background()
	if err := dir.PutMetadata(ctx, SessionMetadata{
		SessionID: "sess-telegram",
		Source:    "telegram",
		ChatID:    "42",
		UserID:    "user-juan",
...
func newTestServiceWithDirectory(t *testing.T) (*TestSqliteStore, *MemMap, *Service, func()) {
	t.Helper()
