...
// localTool is the minimal tool interface for memory tool testing.
...
// localSpec is the optional spec-exposing interface.
type localSpec interface {
	Spec() OperationSpec
}

// testSqlite wraps a *sql.DB with a Close method matching the original SqliteStore.
type testSqlite struct {
	db *sql.DB
}

func (t *testSqlite) DB() *sql.DB { return t.db }
...
func TestMemoryToolsKeepAgentMemoryIndependentInSharedStore(t *testing.T) {
	ctx := context.Background()
	dir := t.TempDir()
	sqlite, err := openTestSqlite(filepath.Join(dir, "memory.db"))
	if err != nil {
...
func newLocalMarkdownToolStore(t *testing.T, ctx context.Context) (*testSqlite, *LocalMarkdownMemoryStore) {
	t.Helper()
	dir := t.TempDir()
	sqlite, err := openTestSqlite(filepath.Join(dir, "memory.db"))
	if err != nil {
...
func executeMemoryTool(t *testing.T, ctx context.Context, tool localTool, args string) map[string]any {
	t.Helper()
