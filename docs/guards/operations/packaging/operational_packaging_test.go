...
func TestServerModeOperationalPackagingFilesStayLocalFirst(t *testing.T) {
	opguard.VerifyFileContracts(t, []opguard.FileContract{
		{Path: "Dockerfile", Markers: []string{"goncho-server", "EXPOSE 8765", "HEALTHCHECK", "127.0.0.1:8765", "useradd --system --uid 10001", "USER goncho"}},
		{Path: "docker-compose.yml", Markers: []string{"goncho-server", "127.0.0.1:8765:8765", "-auth-token", "${GONCHO_SERVER_AUTH_TOKEN:-local-compose-token}", "/data/goncho.db", "goncho-data:/data"}},
		{Path: "scripts/docker_compose_smoke.py", Label: "docker compose smoke wrapper", Markers: []string{"export_module", "smoke.compose.docker_compose"}},
