...
For a narrower public-release-metadata-only check, run `make public-release-smoke`. For a narrower local-go.mod-metadata-only check, run `make local-module-smoke`. For a narrower external-import-only check, run `make public-module-smoke`. For a broader local pre-tag gate, run `make release-smoke`; it wraps release metadata checks and ecosystem smoke with Go tests, vet, race tests, and the docs-site build.
