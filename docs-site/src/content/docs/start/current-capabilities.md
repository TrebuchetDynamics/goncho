...
| Recall trace | `Recall` exposes the scored `RecallTrace` with candidates, provenance, selected/rejected memories, and warnings before projection; `RecallProjector.ProjectContext` preserves selected graph relation-path citations in orientation text | More host-facing replay and review tooling around trace evidence |
