...
Current full-run claim scope: Goncho demonstrates reproducible retrieval performance on LongMemEval-S and LOCOMO while maintaining sub-20ms retrieval latency on multi-thousand-memory corpora. These numbers support retrieval benchmarks, memory ranking quality, long-horizon memory retrieval, deterministic stable-ID scoring, leakage checks, and failure audits. They do not evaluate end-to-end QA, agent task completion, LLM judge scoring, or multi-agent coordination.

Latest full-run evidence:

| Benchmark | Memories | Questions | Key metrics |
| --- | ---: | ---: | --- |
| LongMemEval-S 2026-06-03 | 23,867 | 500 | R@5 `91.12%`, R@10 `94.66%`, recall_any@10 `98.00%`, MRR `91.22%` |
| LOCOMO 2026-06-03 | 5,882 | 1,982 | recall_any@10 `67.91%`, strict@10 `57.67%`, NDCG@10 `49.09%`, MRR `46.89%`, p50/p95 `13ms/18ms` |

The runner itself does not download arbitrary datasets at runtime. It consumes a local JSONL file, loads memories, runs a selected retrieval system, performs leakage checks, writes optional failure audits, and reports:
...
- Latest LOCOMO full-run evidence: `docs/benchmarks/locomo-2026-06-03.md`, `docs/benchmarks/results/locomo-2026-06-03-goncho.json`, and `docs/benchmarks/failures/locomo-2026-06-03-categories.jsonl`.
- Latest LongMemEval-S full-run evidence: `docs/benchmarks/longmemeval-s-2026-06-03.md`, `docs/benchmarks/results/longmemeval-s-2026-06-03-goncho.json`, and `docs/benchmarks/failures/longmemeval-s-2026-06-03-goncho.jsonl`.
