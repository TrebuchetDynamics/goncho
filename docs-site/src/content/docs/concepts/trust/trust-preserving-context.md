...
:::note[Current status]
Current Goncho [Core API](/reference/api/core-api/) surfaces peer cards, search, context assembly, and session summaries, while [memory tools](/reference/memory/memory-tools/) provide the agent-facing memory surface. First-class claim state, confidence, and review products are direction.
:::
