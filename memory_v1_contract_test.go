...
func TestMemoryV1AgentIsolationPolicyMatchesMemoryRecallHelper(t *testing.T) {
	doc, err := ParseGonchoMemoryV1Markdown(mustReadFixture(t, filepath.Join("..", "memory", "testdata", "goncho_v1", "memory.md")))
	if err != nil {
	...
	allowed, reason := CanRecallGonchoMemoryV1(GonchoMemoryV1RecallRequest{
		AgentID:     "agent-b",
